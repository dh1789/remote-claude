...
- **실시간 진행 상황 추적**: 5초 주기 폴링으로 Claude Code 실행 상태 자동 업데이트
- **인터랙티브 버튼**: 상태 확인, 작업 취소, 파일 다운로드를 위한 편리한 버튼 UI
- **대용량 메시지 처리**: 3500자 이상 메시지 자동 분할 전송 (Slack 제한 준수)
...
대부분의 응답 메시지에는 편리한 인터랙티브 버튼이 포함되어 있습니다:

**📊 상태 확인 버튼**
- 클릭 시 현재 채널의 상태와 작업 큐를 즉시 확인
- `/state` 명령과 동일한 기능
...
버튼을 사용하면 명령어를 입력하지 않고도 빠르게 작업을 수행할 수 있습니다.
