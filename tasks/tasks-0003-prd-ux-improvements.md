...
- [ ] 3.0 한글 명령어 매핑 시스템 구현
  - [ ] 3.1 `src/utils/korean-mapper.ts` 파일 생성 및 기본 구조 설정
  - [ ] 3.2 한글 → 영어 자판 변환 테이블 정의 (`KOREAN_TO_ENGLISH_MAP`)
  - [ ] 3.3 한글 자모 분해 함수 구현 (완성형 한글 → 초성/중성/종성 분리)
  - [ ] 3.4 자모 → 영어 자판 매핑 함수 구현 (`convertKoreanToEnglish()`)
  - [ ] 3.5 명령어 매핑 테이블 정의 (`KOREAN_COMMAND_MAP`)
  - [ ] 3.6 `/ㄴㅅㅁ션` → `/state` 매핑 추가
  - [ ] 3.7 `/애쥐ㅐㅁㅇ` → `/download` 매핑 추가
  - [ ] 3.8 한글 명령어 → 영어 명령어 변환 함수 구현 (`mapKoreanCommand()`)
  - [ ] 3.9 매핑되지 않은 명령어 처리 로직 (null 반환)
  - [ ] 3.10 에러 처리 (특수문자, 빈 문자열 등)
  - [ ] 3.11 TypeScript 타입 정의 및 JSDoc 주석 추가
  - [ ] 3.12 `src/index.ts`에 한글 명령어 리스너 추가
  - [ ] 3.13 `/ㄴㅅㅁ션` 슬래시 커맨드 등록 → `/state` 핸들러 호출
  - [ ] 3.14 `/애쥐ㅐㅁㅇ` 슬래시 커맨드 등록 → `/download` 핸들러 호출
  - [ ] 3.15 매핑되지 않은 한글 명령어 입력 시 도움말 메시지 전송
  - [ ] 3.16 README.md에 한글 명령어 사용법 추가
  - [ ] 3.17 `/help` 명령어 출력에 한글 명령어 표시 추가
